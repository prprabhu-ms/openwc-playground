...
const template = html<PrprabhuReconciliationBranching>`
  <div>
    ...
    <p>
      Inline template return from <code>when</code> directive. This one is
      surprising. FAST decides to cache the DOM nodes even though they are
      removed by <code>when</code> and the template itself is dynamic.
      ${when(
        x => x.left,
    ...
    <p>Select global template. ${x => x.selectCounterTemplate()}</p>
    <p>
      Select dynamic template. Same as global template (DOM nodes removed), but
      should be worse in performance because template will be registered each
      time. ${x => x.selectCounterTemplateDynamic()}
    </p>
...
@customElement({
  name: 'prprabhu-reconciliation-branching',
  template,
  styles: commonStyles,
})
